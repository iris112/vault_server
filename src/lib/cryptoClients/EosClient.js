...
const log = debug('CCVault:EosClient')

export default class EosClient extends CryptoClient {
  ...
  constructor(config) {
    ...
    this.signatureProvider = new JsSignatureProvider([this.getBankPK()]);
  ...
  async getBalance(account = this.getBankAddress()) {
    return parseFloat(await this.rpc.get_currency_balance('eosio.token', account, 'EOS'))
  }
  ...
  async sendToAddress(toAccount, amount, memo) {
      ...
      actions: [
        {
          ...
          name: 'transfer',
          authorization: [{
            actor: this.getBank().accountName,
            permission: 'active',
          }],
          data: {
            from: this.getBank().accountName,
            to: toAccount,
  ...
  isValidAction(action) {
    return action.name === 'transfer' &&
      action.data.quantity && action.data.quantity.length &&
      !isNaN(action.data.memo) &&
      action.data.to === this.getBankAddress()
  }
